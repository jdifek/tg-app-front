...
export default function PayPalPage() {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      await apiFetch(`/api/orders/${orderId}/payment-status`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ paymentStatus: "AWAITING_CHECK" }),
      });

      // Отправляем скриншот через FormData
      const formData = new FormData();
      formData.append("screenshot", screenshot);

      const response = await apiFetch(`/api/orders/${orderId}`, {
        method: "PATCH",
        body: formData, // НЕ ставим headers
      });

      if (!response.ok) {
        const err = await response.json();
        throw new Error(err.error || "Не удалось прикрепить скриншот");
      }
        ...
        <div className="bg-gray-900 bg-opacity-50 border border-pink-600 rounded-2xl p-5">
          ...
          <ul className="space-y-2 text-sm leading-relaxed">
            ...
            <li>
              ...
              <span className="text-pink-400"> friend, girlfriend, gift, present, donate</span>
            </li>
          </ul>
          ...
          <div className="mt-4">
            ...
            <div className="border border-dashed border-gray-600 rounded-xl p-4 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-gray-800 transition">
              <input
                ...
                id="screenshotUpload"
              />
              <label htmlFor="screenshotUpload" className="cursor-pointer flex flex-col items-center">
